...
- **victory** — when the season is won, and when it merely ends (§10).
  ...
  ([decision 0020](../decisions/0020-blocs-are-transitive-and-winning-stops-nothing.md)):
  a bloc holding 40% of all provinces starts a seven-in-game-day clock that
  resets if it slips or changes shape; at `SEASON_TICKS` an unwon season
  ends on points — provinces, industry, trust, scored per bloc. `season_won`
  is emitted once, the state remembers it for ever, and the world goes on
  turning: archiving is an operator's act, not a tick's. The `victory` view
  rides publicly on every full state and delta.
...
| `src/server/`               | new      | The world server: `world/` (World, WorldState and its reducer, TickLoop, WorldRunner), `systems/` (economy, construction, production, research, trade, supply, air, naval, combat, regent, victory, routes — every §6 system is real now), `db/` (store interface, memory and Postgres store), `net/` (socket and health). |
