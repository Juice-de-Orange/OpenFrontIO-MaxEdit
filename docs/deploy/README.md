...
| `REGENT_FOCUS_RESEED` | _(unset)_   | Operator one-off on a season world: `1` moves every regent-run nation's focus onto the seed's draw at startup (Season.ts), through the command log; claimed nations are untouched. Set it, start once, remove it — a second start finds nothing to move.          |
...
rsync -az resources/flags/ HOST:/srv/world/www/flags/
```

**And the flags, separately.** Since protocol 17 the client shows each
